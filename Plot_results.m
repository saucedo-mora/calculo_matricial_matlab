function Plot_results(force_ini, displacement, num_ele, force, A, E, nod_coor_def, I, num_nod, L, magni_flechas, stress, ratio_plot, nod_coor, ele_nod)
    %figure
    %subplot(2,1,1);
    ...
    for e=1:num_nod
        ...
        if ((force_ini((e-1)*3+1)^2)+abs(force_ini((e-1)*3+2))^2)^0.5>0
            indsize=((((force_ini((e-1)*3+1)^2)+(force_ini((e-1)*3+2))^2)^0.5)/maxforce)^0.25
            q=quiver(prefx,prefy, force_ini((e-1)*3+1)/((force_ini((e-1)*3+1)^2)+abs(force_ini((e-1)*3+2))^2)^0.5,force_ini((e-1)*3+2)/((force_ini((e-1)*3+1)^2)+abs(force_ini((e-1)*3+2))^2)^0.5,indsize*magni_flechas,'LineWidth',5,'MaxHeadSize',15)
            q.Color = 'k';
        end
        if abs(force_ini((e-1)*3+3)) >0
            scatter(prefx,prefy,0.5*magni_flechas,'LineWidth',3,'MarkerEdgeColor','k')
        end
    end
    ...
    for e=1:num_nod
        prefx=nod_coor_def(e,1)
        prefy=nod_coor_def(e,2)
        if ((freact((e-1)*3+1)^2)+(freact((e-1)*3+2)^2))^0.5 >0
            indsize=((((freact((e-1)*3+1)^2)+(freact((e-1)*3+2)^2))^0.5)/maxforce)^0.25;
            q=quiver(prefx,prefy, freact((e-1)*3+1)/((freact((e-1)*3+1)^2)+(freact((e-1)*3+2)^2))^0.5,freact((e-1)*3+2)/((freact((e-1)*3+1)^2)+(freact((e-1)*3+2)^2))^0.5,indsize*magni_flechas,'LineWidth',5,'MaxHeadSize',15);
            q.Color ='r'; %[0.75, 0, 0.75];
           % text(prefx,prefy,num2str(((freact((e-1)*2+1)^2)+(freact((e-1)*2+2)^2))^0.5,1),'FontSize',15)
        end
        if abs(freact((e-1)*3+3)) >0
            scatter(prefx,prefy,0.5*magni_flechas,'LineWidth',3,'MarkerEdgeColor','r')
        end
    end
    ...
    for e=1:num_ele
        displa_elx(e,1)=0.5*displacement((ele_nod(e,1)-1)*3+1,1)+0.5*displacement((ele_nod(e,2)-1)*3+1,1)
    end
    ...
    displa_ely=zeros(num_ele,1);
    for e=1:num_ele
        displa_ely(e,1)=0.5*displacement((ele_nod(e,1)-1)*3+2,1)+0.5*displacement((ele_nod(e,2)-1)*3+2,1);
    end
    ...
    strmax=-1000000000.0;
    strmin=10000000000.0;
    for e=1:num_ele  % calculo energia
        valstr=0.5*stress(e)*stress(e)*L(e)*A(e)/E(e)+0.5*((displacement((ele_nod(e,1)-1)*3+3,1)-displacement((ele_nod(e,2)-1)*3+3,1))^2)*E(e)*I(e)/L(e);
        if valstr>strmax
            strmax=valstr;
        end
        if valstr<strmin
    ...
    for e=1:num_nod
        ...
        if ((freact((e-1)*3+1)^2)+(freact((e-1)*3+2)^2))^0.5 >0
            ...
            q=quiver(prefx,prefy, freact((e-1)*3+1)/((freact((e-1)*3+1)^2)+(freact((e-1)*3+2)^2))^0.5,freact((e-1)*3+2)/((freact((e-1)*3+1)^2)+(freact((e-1)*3+2)^2))^0.5,indsize*magni_flechas,'LineWidth',5,'MaxHeadSize',15)
            q.Color ='r'%[0.75, 0, 0.75];
            text(prefx,prefy,num2str(((freact((e-1)*3+1)^2)+(freact((e-1)*3+2)^2))^0.5,1),'FontSize',15)
        end
        if abs(freact((e-1)*3+3)) >0
            scatter(prefx,prefy,0.5*magni_flechas,'LineWidth',3,'MarkerEdgeColor','r')
        end
    ...
    figure('NumberTitle', 'off', 'Name', 'Giro');
    % undeformed truss plot
    for e=1:num_ele
        x=[nod_coor(ele_nod(e,1),1) nod_coor(ele_nod(e,2),1)];
        y=[nod_coor(ele_nod(e,1),2) nod_coor(ele_nod(e,2),2)];
    ...
    strmin=10000000000.0;
    for e=1:num_ele  % calculo energia
        valstr=abs(displacement((ele_nod(e,1)-1)*3+3,1)-displacement((ele_nod(e,2)-1)*3+3,1));
        if valstr>strmax
            strmax=valstr;
        end
        if valstr<strmin
    ...
    char5=num2str(strmin+1*(strmax-strmin),1);
    c = colorbar('southoutside','Ticks', [0, 0.25, 0.5, 0.75, 1],'TickLabels',{char1, char2, char3, char4, char5});
    c.Label.String = 'Giro en valor absoluto (rad)';
    c.FontSize = 10; 
